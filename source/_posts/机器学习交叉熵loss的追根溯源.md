...
f(x|p)=p^{x}(1-p)^{1-x}
$$
那么最大似然是在做一件什么事？它希望依据目前的情况来推演$p$的值，使得当前事件发生的概率最大。也就是说假设我们观测到了一个随机过程，比如$x={x_1,x_2,x_3, \cdots,x_n}$，每一个$x_i$对应的都是一个随机变量$X$，他们之间相互独立。对于抽卡来说，就是我们抽了$n$次卡，那么最大似然的意思就是希望刚刚我们看到的事情发生的情况发生的概率最大。
$$
p = \mathop{\operatorname{argmax}}\limits_{p} \prod_{i=1}^n f(x_i|p)
$$
因为$x_i$都是已经发生的，那么只有$p$是变量，之后就是求极值了。那么这个式子为什么这样呢？其实这本身蕴含了一种假设，就是**已经发生的事件发生的概率是最大的**这个假设。亦如大家抽卡时的思想"我抽了100抽都没中，卡池里根本没有这张卡！！($p=0)$"。
